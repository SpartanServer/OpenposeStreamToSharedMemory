...
public:
    void initializationOnThread() {
		for (int i = 0; i < 25; i++) {
			char name[100]="";
			strcat(name, FLAGS_memname.data());
			strcat(name, "body");
			strcat(name, std::to_string(i).data());
			std::cout << name << std::endl;
			hSharedMemory[i]= CreateFileMapping(NULL, NULL, PAGE_READWRITE, NULL, sizeof(point_t), name);
		}
        ...
        try
        {
            if (datumsPtr != nullptr && !datumsPtr->empty())
            {
                ...
                for (auto person = 0 ; person < poseKeypoints.getSize(0) ; person++)
                {
                    ...
                    {
						point_t* pMemory = (point_t*)MapViewOfFile(hSharedMemory[bodyPart], FILE_MAP_ALL_ACCESS, NULL, NULL, sizeof(point_t));
                        point_t valueToPoint;
                        for (auto xyscore = 0 ; xyscore < poseKeypoints.getSize(2) ; xyscore++)
                        {
							if (xyscore == 0) valueToPoint.x = poseKeypoints[{person, bodyPart, xyscore}];
							if (xyscore == 1) valueToPoint.y = poseKeypoints[{person, bodyPart, xyscore}];
							if (xyscore == 2) valueToPoint.c = poseKeypoints[{person, bodyPart, xyscore}];
                        }
						*pMemory = valueToPoint;
                    }
                }
